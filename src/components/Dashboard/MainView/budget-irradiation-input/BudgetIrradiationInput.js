...
import "./BudgetIrradiationInput.css";
...
import IrradiationComparison from "./irradiation-comparision/IrradiationComparison";
import IrradiationInput from "./irradiation-input/IrradiationInput";
export default function BudgetIrradiationInput(props) {
  ...
  const handleConfirmAddRecord = () => {
    ...
    axios
      .post(`/entityupdate`, {
        id: siteselect[1],
        data: {
          irradiationBudget: valuerecordscopy,
        },
      })
      .then((response) => {
        ...
        axios.get(`/entityget?id=${id}`).then((response) => {
          if (response.data) {
            setSiteselect([
              response.data.name,
              response.data.id,
              response.data.irradiationBudget,
            ]);
          }
        });
      });
  ...
  return (
    <div className="budget-production-input">
      <div className="budget-production-input-background"></div>
          ...
          <div className="budget-production-input-header-sitedisplay-dropdown">
            {entities.map((item, index) => {
              return (
                ...
                >
                  <Link
                    to={`/dashboard/site-management/budgetinsolationinput/${item.id}`}
      ...
      <div className="budget-production-input-body">
        <div className="budget-production-input-body-row-1">
          <IrradiationComparison productionBudget={siteselect[2]} siteid={id} />
        </div>
